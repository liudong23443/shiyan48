        ...
        with st.spinner("计算预测结果..."):
            try:
                ...
                <div style="text-align: center; margin: -0.2rem 0 0.3rem 0;">
                    <span style="font-size: 1.1rem; font-family: 'Microsoft YaHei', 'SimHei'; color: {risk_color}; font-weight: bold;">
                        {risk_category}
                    </span>
                </div>
                """, unsafe_allow_html=True)
                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        # 调用我们的自定义函数生成SHAP瀑布图
                        # 根据SHAP值的形状确定是二分类还是多分类
                        is_binary = not (hasattr(shap_values, 'values') and len(shap_values.values.shape) > 2)
                        generate_shap_plot(model, features_df, model.feature_names_in_, shap_values, is_binary)
...
# 在瀑布图生成前再次设置字体，确保应用
def generate_shap_plot(model, input_example, feature_names, shap_values=None, is_binary=True):
    ...
    # 确保图形大小合适 - 使用与原始代码一致的尺寸
    plt.figure(figsize=(6.5, 3), dpi=200, facecolor='#f8f9fa')
    
    # 使用已计算的SHAP值或计算新的SHAP值
    ...
    # 添加标题
    plt.title("特征对预测的影响", fontsize=12, fontproperties=FontProperties(family='Microsoft YaHei', weight='bold'), color='black')
    
    # 获取当前图形并修改字体设置
    ax = plt.gca()
    for text in ax.texts:
        text.set_fontproperties(FontProperties(family='Microsoft YaHei'))
